...
function estaAutenticado() {
    return isset(_SESSION['user_id']) && !empty(
_SESSION['user_id']);
}
...
function redirigir($url) {
    if (!preg_match('/^https?:\/\//', $url)) {
        url=BASEURL.′/′.ltrim(url = BASE_URL . '/' . ltrim(
url=BASEU​RL.′/′.ltrim(url, '/');
    }
    header('Location: ' . $url);
    exit;
}
...
function formatearFecha($fecha, $formato = 'd/m/Y H:i') {
    ...
    timestamp=isnumeric(timestamp = is_numeric(
timestamp=isn​umeric(fecha) ? fecha:strtotime(fecha : strtotime(
...
function diasRestantes($fechaLimite) {
    $ahora = time();
    limite=isnumeric(limite = is_numeric(
limite=isn​umeric(fechaLimite) ? fechaLimite:strtotime(fechaLimite : strtotime(
